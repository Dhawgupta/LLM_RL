    ...
    def mc_data_generator(data_name):
        with open(data_name, "r") as f:
            for item in f:
                ...
                last_trajectory = TextTrajectory([Text(obj[-1]["from_state"], False), Text(obj[-1]["action"], True)], 
                                                 [0, obj[-1]["reward"]], True)
                ...
                for traj in reversed(obj): # iterate through move history backwards except for last transition
                    # embed()
                    prev_trajectory = TextTrajectory([Text(traj["from_state"], False), Text(traj["action"], True)], 
