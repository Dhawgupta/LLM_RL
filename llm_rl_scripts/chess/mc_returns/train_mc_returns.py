...
from JaxSeq.utils import BlockingStrategy, Padding, Truncation, get_weight_decay_mask, MapIterable
import os
import optax
...
import re
from LLM_RL.environment import TokenTrajectoryChain, text_history_to_str
...
from llm_rl_scripts.chess.env.data import get_random_positions_not_in_test, get_saved_text_chains
...
def main(
    ...
    log_every: int=256, 
    eval_every_steps: Optional[int]=1024, 
    ...
    def map_text_trajectory_chain(chain):
    ...
    train_dataset = MCIterableDataset.from_mc_data_iterable(
        MapIterable(map_text_trajectory_chain, train_data),
        tokenizer,
        BlockingStrategy(
    ...
    eval_dataset = MCIterableDataset.from_mc_data_iterable(
        MapIterable(map_text_trajectory_chain, eval_data),
        tokenizer,
        BlockingStrategy(
            padding=Padding.RIGHT,
            truncation=Truncation.RIGHT,
