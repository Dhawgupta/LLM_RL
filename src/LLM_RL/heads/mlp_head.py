...
class MLPHead(nn.Module):
    ...
    def pad_outputs(self, params: PyTree, param_sharding: Optional[PyTree]=None, dtype: jnp.dtype=jnp.float32) -> PyTree:
        ...
        out_kernel = jnp.zeros((self.config.hidden_dim, self.config.output_dim), dtype=dtype)
        params["dense2"]["kernel"] = out_kernel.at[:, :self.config.unpadded_output_dim].set(params["dense2"]["kernel"])
        if param_sharding is not None:
            params["dense2"]["kernel"] = jax.device_put(
                params["dense2"]["kernel"], 
                param_sharding["dense2"]["kernel"], 
            )
        out_bias = jnp.zeros((self.config.output_dim,), dtype=dtype)
        params["dense2"]["bias"] = out_bias.at[:self.config.unpadded_output_dim].set(params["dense2"]["bias"])
        if param_sharding is not None:
            params["dense2"]["bias"] = jax.device_put(
                params["dense2"]["bias"], 
