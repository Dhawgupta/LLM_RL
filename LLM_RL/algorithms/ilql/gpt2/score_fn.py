...
def build_ilql_score_fn(
    ...
    def score_fn(text_histories: List[TextHistory]) -> List[float]:
        ...
        for i in range(0, len(text_histories), bsize):
            ...
            qs = jnp.minimum(values['target_q1'], values['target_q2'])
            ...
            for x in range(len(prefix_len)):
                action_advs = action_advs.at[x].set(value_weight * ((qsa[x] - values['v'][x, :-1].squeeze(1)) * attention_mask[x, 1:])[(prefix_len[x]-1):].sum(axis=0))
