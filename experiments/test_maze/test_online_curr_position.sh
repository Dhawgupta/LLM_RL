...
do 
    python llm_rl_scripts/maze/train_ppo_online.py HF gcs://rail-tpus-isadora/maze/maze_double_t_maze/double_t_maze_give_position_bc/model_141000 --n-rounds 50 --exp-name ppo_"$maze_name"_give_position_no_reranker_num_epochs_"$num_epochs"kl_coeff_0.01_wd_0.01 --wandb-project ppo_"$maze_name" --model-mesh-shape 2 --data-mesh-shape 1 --fsdp-mesh-shape 1 --n-rollouts 256 --ppo-data-bsize 8 --use-wandb --train-bsize 8 --grad-accum-steps 16 --lr 1e-6 --maze-name "$maze_name" --describe-function "describe_observation_give_position" --init-kl-coef 0.01 --epochs "$num_epochs" --weight-decay 0.01
done
