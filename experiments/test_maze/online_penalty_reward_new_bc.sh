...
do 
    ...
    do 
        python llm_rl_scripts/maze/train_ppo_online.py HF gcs://rail-tpus-isadora/maze/maze_double_t_maze/double_t_maze_give_position_bc_submazes/model_145001 --n-rounds 100 --save-every-rounds 5 --exp-name ppo_"$maze_name"_give_position_penalty_num_epochs_"$num_epochs"kl_coeff_0.001_wd_0.01_lr_1e-6_gamma_"$gamma"_1024_rollouts --wandb-project ppo_"$maze_name" --model-mesh-shape 2 --data-mesh-shape 1 --fsdp-mesh-shape 1 --n-rollouts 1024 --ppo-data-bsize 8 --use-wandb --train-bsize 8 --grad-accum-steps 16 --lr 1e-6 --maze-name "$maze_name" --describe-function "describe_observation_give_position" --init-kl-coef 0.001 --epochs "$num_epochs" --weight-decay 0.01 --reward-function "illegal_penalty_diff_scale" --gamma "$gamma"
    done
done
